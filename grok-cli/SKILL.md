---
name: grok-cli
description: Use the locally installed Grok CLI (`grok`) for headless one-shot prompts, repo-aware coding/review prompts, model listing, and lightweight automation on this machine. Trigger when the user explicitly asks to use Grok CLI / grok-cli / `grok`, wants a local Grok run instead of OpenClaw's built-in xAI tools, wants a prompt executed inside Grok against a working directory, or wants Grok CLI-specific behavior like `grok -p`, `grok models`, or resuming a Grok session.
---

# Grok CLI

Use the local `grok` binary on this machine for direct Grok CLI workflows.
...
- Default workspace to use unless the user specifies another path: `/home/deck/.openclaw/workspace`
...
- Default to `/home/deck/.openclaw/workspace` if the user does not specify a path.
...
If a run may take longer, give `exec` a larger `yieldMs` or poll the spawned process instead of tight loops.

## Bundled resources

- Script: `scripts/grok-headless-clean.sh`
  - Use for headless text prompts with ANSI/progress/session noise stripped
...
Do not use Grok CLI when OpenClaw first-class tools are better:

- Use `web_search` for ordinary web research
- Use `x_search` for X/Twitter search
- Use `image_generate` / `image` for OpenClaw-native image flows
